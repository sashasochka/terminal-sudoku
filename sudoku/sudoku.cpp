...
void Sudoku::fill_random()
{
    ...
    {
        try{
            field.assign(field_side_size,field_side_size);
            for(int k=0; k<field_side_size*field_side_size; k++)
            {
                Coord coord=min_variants();
                if(!put_random(coord.x,coord.y)) throw 1;
            }
            break;
        }
        catch(int err)
...
void Sudoku::random_copy2userfield()
{
    ...
    userfield.assign(field_side_size,field_side_size);
...
void Sudoku::user_out(ostream& out_stream)
{
    ...
    for(int i=0; i<field_side_size; i++)
    {
        ...
        {
            out_stream.width(field_side_size>9?3:1);
            if(i==last_changed.y && j==last_changed.x)   // last changed - red
                set_text_color(0xDA);
            else if(userfield[i][j].value)               // if value!=0
                set_text_color( userfield[i][j].status!=closed ? 0xA : 0xe ); 
            else                                         // if value==0
                set_text_color(0x7);
            out_stream<<userfield[i][j].value;
            set_text_color();
            if( !( (j+1)%m_side_size ) && j!=field_side_size-1) cout<<"|";
        }
        set_text_color();
        out_stream<<endl;
        if( !( (i+1)%m_side_size ) && i!=field_side_size-1) cout<<nl<<endl;
    }
    set_text_color(7);
}
...
{
    for(int i=0; i<field_side_size; i++)
        if( field[row_index][i]==value) return true;
    return false;
}
...
{
    for(int i=0; i<field_side_size; i++)
        if( field[i][column_index]==value) return true;
    return false;
}
...
void Sudoku::set_text_color(short color)
{
    if(!color)
    {
        system("color");
...
bool Sudoku::is_won()
{
    for(int y=0; y<field_side_size; y++)
    {
        for(int x=0; x<field_side_size; x++)
        {
            if(!userfield[y][x].value) return false;
